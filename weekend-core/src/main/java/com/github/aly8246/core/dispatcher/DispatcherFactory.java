...
import static com.github.aly8246.core.dispatcher.AbstractDispatcher2.regxListParamClass;
...
public class DispatcherFactory {
    ...
    public DispatcherFactory(Object proxy, Method method, Object[] args) {
        this.proxy = proxy;
        this.method = method;
        this.args = args;
    ...
    private void init() throws IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        Object returnClassType = ResultCase.getInstance(method.getReturnType());
        ...
        if (!command.returnType().equals(Collection.class)) {
            //如果查询注解指定了返回对象
            realClassType = command.returnType().newInstance();
            realClassType=new ArrayList<>();
        } else {
...
//        Dispatcher dispatcher = new SimpleDispatcher2(this.proxy, this.method, this.args, this.retClass);
//
//        //如果包含结果集映射
//        if (this.retClass.containerMapping())
//            dispatcher = new CustomDispatcher2(this.proxy, this.method, this.args, this.retClass);
        Dispatcher dispatcher = new InitializerDispatcher(proxy, method, args);
        return dispatcher;
    }
